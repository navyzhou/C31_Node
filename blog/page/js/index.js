...
function showPicCycle(){
		...
		if(num<200){ //如果小于200，则说明是要查看上张图片
			if(index<=0){ //如果已经是第一张，则提示
				...
				if(index!=0 && index%8==0){
					$("#navy_show_imgsmall_view").animate({scrollLeft:((index/8-1)*560)},300);
				}
			...
			if(index>=total-1){ //判断是否已经到了最后一张
				...
				if(index!=0 && index%8==0){
					...
					$("#navy_show_imgsmall_view").animate({scrollLeft:(index/8*560)},300);
				}
			}
		}
		showPic(index);
	...
	$("#navy_right").click(function(){
		...
		if(total>8){
			...
			if(num<=page){
				$("#navy_show_imgsmall_view").animate({scrollLeft:560*num},300);
				index=num*8;
				showPic(index);
			}
		}
	...
	$("#navy_left").click(function(){
		...
		if(left>=592){ //说明至少向右翻了一页
			$("#navy_show_imgsmall_view").animate({scrollLeft:left-560},300) ;//则向右释放592
			index=(Math.floor(left/560)-1)*8;
			showPic(index);
		}
	});
